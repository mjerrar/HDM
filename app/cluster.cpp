...
ClusterExtractor::ClusterExtractor(float tolerance, int minClusterSize,
                                   int maxClusterSize) {
 ...
 */
void ClusterExtractor::initEuclideanClustractor(float tolerance,
                                                int minClusterSize,
                                                int maxClusterSize) {
...
bool ClusterExtractor::extract(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud) {
  ...
  int j = 0;
  for (std::vector<pcl::PointIndices>::const_iterator it = this->cluster_indices
      .begin(); it != this->cluster_indices.end(); ++it) {
